...
function App() {
  ...
  function handleSerialText(text: string) {
    ...
    if (rpmTestModeMatch) {
      ...
      setNotice(`Dyno RPM pin test: ${enabled ? 'enabled' : 'disabled'}`)
      return
    }
...
function SerialConsolePanel({ messages, showSensorData, autoScroll, customCommand, setCustomCommand, onToggleSensorData, onToggleAutoScroll, onClear, onSendCommand, onSendCustom, rpmPinTest, rpmPinStates, onToggleRpmPinTest }: { messages: ConsoleMessage[]; showSensorData: boolean; autoScroll: boolean; customCommand: string; setCustomCommand: (value: string) => void; onToggleSensorData: () => void; onToggleAutoScroll: () => void; onClear: () => void; onSendCommand: (bytes: Uint8Array, description?: string) => Promise<void>; onSendCustom: () => Promise<void>; rpmPinTest: boolean; rpmPinStates: [boolean | null, boolean | null]; onToggleRpmPinTest: () => Promise<void> }) {
